...
async function loadGraph() {
  const s = await getJSON("/api/dashboard/graph");
}

async function load() {
  await Promise.all([loadRate(), loadGraph()]);
  // // 1) 자산 + 환율 데이터
  ...
  // document.getElementById("rate_asof").textContent = s.as_of
  //   ? `${s.as_of}`
  //   : "";
  // // 총 자산 (USD / KRW 함께 표시)
  // document.getElementById("total_both").innerHTML = `${fmtUSD(
  //   num(s.total.amount_usd)
  // )}<br>${fmtKRW(num(s.total.amount_krw))}`;
  // // USD 자산
  // document.getElementById("total_usd").innerHTML = `${fmtUSD(
  //   num(s.breakdown.usd_assets.amount_usd)
  // )}<br>${fmtKRW(num(s.breakdown.usd_assets.amount_krw))}`;
  // document.getElementById("usd_percent").textContent = `(${num(
  //   s.breakdown.usd_assets.percent
  // ).toFixed(1)}%)`;
  // // KRW 자산
  // document.getElementById("total_krw").innerHTML = `${fmtUSD(
  //   num(s.breakdown.krw_assets.amount_usd)
  // )}<br>${fmtKRW(num(s.breakdown.krw_assets.amount_krw))}`;
  // document.getElementById("krw_percent").textContent = `(${num(
  //   s.breakdown.krw_assets.percent
  // ).toFixed(1)}%)`;
  // // --- USD 계좌 렌더링 ---
  // const usd_tbody = document.getElementById("usd_table");
  // let usd_total_usd = 0,
  //   usd_total_krw = 0;
  // let usd_total_cash = 0,
  //   usd_total_saving = 0,
  //   usd_total_bond = 0,
  //   usd_total_stock = 0;
  // let usd_total_invested = 0,
  //   usd_total_profit = 0;
  // let total_krw_net = 0;
  // let total_usd_net = 0;
  // let total_investment = 0;
  // const order = { Stock: 0, Saving: 1, Checking: 2 };
  // const usd_sorted = s.usd_accounts
  //   .map((acc, idx) => {
  //     return {
  //       ...acc,
  ...
  //       fx_effect_html = `${fx_only_effect_pct.toFixed(
  //         2
  //       )}%<br><span class="text-xs text-gray-500">(${avg_fx.toFixed(
  //         2
  //       )})</span>`;
  //       total_investment += acc.networth_usd;
  //       fx_return_amt_html = fmtKRW(acc.networth_usd * s.rate - acc.krw_net);
  //       const fx_return_pct =
  //         ((acc.networth_usd * s.rate - acc.krw_net) / acc.krw_net) * 100;
  ...
  // document.getElementById("usd_sum_saving").textContent =
  //   fmtUSD(usd_total_saving);
  // document.getElementById("usd_sum_bond").textContent = fmtUSD(usd_total_bond);
  // document.getElementById("usd_sum_stock").textContent =
  //   fmtUSD(usd_total_stock);
  // document.getElementById("usd_sum_krw").textContent = fmtKRW(usd_total_krw);
  // document.getElementById("usd_sum_invested_usd").textContent =
  //   fmtUSD(usd_total_invested);
  // document.getElementById("usd_sum_return_amt").textContent =
  //   fmtUSD(usd_total_profit);
  ...
  //     "usd_sum_fx_effect_pct"
  //   ).innerHTML = `${fx_only_effect_pct.toFixed(
  //     2
  //   )}%<br><span class="text-xs text-gray-500">(${avg_fx.toFixed(2)})</span>`;
  ...
  //     fmtKRW(fx_return_amt_total);
  //   // 환율 고려 수익률(%) = (수익금액 / 원금(KRW)) × 100
  //   const fx_return_pct_total = (fx_return_amt_total / total_krw_net) * 100;
  //   document.getElementById(
  //     "usd_sum_fx_return_pct"
  //   ).textContent = `${fx_return_pct_total.toFixed(1)}%`;
  // } else {
  ...
  // let krw_total_krw = 0,
  //   krw_total_usd = 0;
  // let krw_total_cash = 0,
  //   krw_total_saving = 0,
  //   krw_total_bond = 0,
  //   krw_total_stock = 0;
  // let krw_total_invested = 0,
  //   krw_total_profit = 0;
  // const krw_sorted = s.krw_accounts
  //   .map((acc, idx) => {
  //     return {
  //       ...acc,
  ...
  //   });
  // krw_tbody.innerHTML = krw_sorted
  //   .map((acc) => {
  //     const invested = num(acc.breakdown?.invested);
  //     const profit = num(acc.breakdown?.profit);
  //     const return_pct = invested
  ...
  // document.getElementById("krw_sum_saving").textContent =
  //   fmtKRW(krw_total_saving);
  // document.getElementById("krw_sum_bond").textContent = fmtKRW(krw_total_bond);
  // document.getElementById("krw_sum_stock").textContent =
  //   fmtKRW(krw_total_stock);
  // document.getElementById("krw_sum_usd").textContent = fmtUSD(krw_total_usd);
  // document.getElementById("krw_sum_invested_krw").textContent =
  //   fmtKRW(krw_total_invested); // ✅ 추가
  // document.getElementById("krw_sum_return_amt").textContent =
  //   fmtKRW(krw_total_profit);
  ...
  //           const usdText = `$${value.toLocaleString("en-US", {
  //             maximumFractionDigits: 0,
  //           })} (${percentage})`;
  //           const krwValue = value * s.rate;
  //           const krwText = `₩${krwValue.toLocaleString("ko-KR", {
  //             maximumFractionDigits: 0,
  //           })}`;
  //           return `${usdText}\n${krwText}`;
  //         },
